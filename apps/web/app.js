...
async function loadProfiles() {
  try {
    const res = await fetch(`${apiBase}/profiles/active`);
    const data = await res.json();
    ...
    for (const profile of data.profiles) {
      ...
      div.innerHTML = `
        ...
        <div>
          <strong>${escapeHtml(profile.display_name)} · ${escapeHtml(profile.public_id)}</strong>
          <small>${escapeHtml(profile.intent)} · ${escapeHtml(profile.zone)} · ${escapeHtml(profile.tags.join(', '))}</small>
